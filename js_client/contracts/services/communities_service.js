...
export class CommunitiesService {
  ...
  async postFriendRequest({ body, bearerToken } = {}) {
    const url = new URL(`${this.baseUrl}/communities/friend-request`);
    ...
    return DTOResponse.fromJson(json, (data) => FriendRequestResponseDTO.fromJson(data));
  }

  async postRespond({ body, bearerToken } = {}) {
    const url = new URL(`${this.baseUrl}/communities/friend-request/respond`);
    const headers = { 'Content-Type': 'application/json' };
    if (bearerToken) headers['Authorization'] = `Bearer ${bearerToken}`;
    const response = await this.fetcher(url.toString(), {
      method: 'POST',
  ...
  async getFriendRequests({ bearerToken } = {}) {
    const url = new URL(`${this.baseUrl}/communities/friend-requests`);
    const headers = { 'Content-Type': 'application/json' };
    if (bearerToken) headers['Authorization'] = `Bearer ${bearerToken}`;
    const response = await this.fetcher(url.toString(), {
      method: 'GET',
    ...
    const json = await response.json();
    return DTOResponse.fromJson(json, (data) => FriendRequestListResponseDTO.fromJson(data));
  }

  async getUsers({ bearerToken } = {}) {
    const url = new URL(`${this.baseUrl}/communities/users`);
    const headers = { 'Content-Type': 'application/json' };
    if (bearerToken) headers['Authorization'] = `Bearer ${bearerToken}`;
    const response = await this.fetcher(url.toString(), {
      method: 'GET',
    ...
    const json = await response.json();
    return DTOResponse.fromJson(json, (data) => UserListResponseDTO.fromJson(data));
  }
